...
class ContentDiscriminator(nn.Module):
    def __init__(self, gpu_ids=[]):
        ...
        model = []
        model += [self.leaky_relu_conv2d(256, 256, kernel_size=7, stride=2, padding=1, norm='Instance')]
        model += [self.leaky_relu_conv2d(256, 256, kernel_size=7, stride=2, padding=1, norm='Instance')]
        model += [self.leaky_relu_conv2d(256, 256, kernel_size=7, stride=2, padding=1, norm='Instance')]
        model += [self.leaky_relu_conv2d(256, 256, kernel_size=4, stride=1, padding=0)]
        model += [nn.Conv2d(256, 1, kernel_size=1, stride=1, padding=0)]
        self.model = nn.Sequential(*model)

    def leaky_relu_conv2d(self, n_in, n_out, kernel_size, stride, padding=0, norm='None'):
        layers = []
        layers += [nn.ReflectionPad2d(padding)]
        layers += [nn.Conv2d(n_in, n_out, kernel_size=kernel_size, stride=stride, padding=0, bias=True)]
        if norm == 'Instance':
            layers += [nn.InstanceNorm2d(n_out, affine=False)]
    ...
    def compute_loss(self, x, gt):
        x = self.forward(x)
        gt = self.forward(gt)
        for it, (out_a, out_b) in enumerate(zip(x, gt)):
            out_fake = nn.functional.sigmoid(out_a)
            out_real = nn.functional.sigmoid(out_b)
            device = out_real.device
            all1 = torch.ones((out_real.size(0))).to(device)
            all0 = torch.zeros((out_fake.size(0))).to(device)
            ad_true_loss = nn.functional.binary_cross_entropy(out_real, all1)
            ad_fake_loss = nn.functional.binary_cross_entropy(out_fake, all0)
            loss_D = ad_true_loss + ad_fake_loss
            return loss_D

    def compute_GAN_content_loss(self, data):
        outs = self.forward(data)
        for out in outs:
            outputs_fake = nn.functional.sigmoid(out)
            device = outputs_fake.device
            all_half = 0.5*torch.ones((outputs_fake.size(0))).to(device)
...
class StyleEncoderShared(nn.Module):
    def __init__(self, dim=64, style_dim=8):
        super(StyleEncoderShared, self).__init__()
        dim = dim * 4
        layers = []

        for _ in range(3):
            layers += [nn.Conv2d(dim, dim, 3, stride=1, padding=1), nn.ReLU(inplace=True)]
